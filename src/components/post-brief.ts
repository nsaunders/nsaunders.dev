...
import { component, div, h1, img, p } from "renuel";
...
import { _Link } from "./link.ts";
import { TextLink$ } from "./text-link.ts";

export const { PostBrief } = component(
  "PostBrief",
  (
    props: Pick<
    ...
    return div(
      ...
      div(
        ...
        h1(
          ...
          TextLink$(_Link({ to: `/posts/${props.name}` }, props.title)),
        ),
        p({ style: { margin: 0 } }, props.description),
        div(
          {
            style: pipe(
              {

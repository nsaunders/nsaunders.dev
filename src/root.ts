...
import { Vr } from "./components/vr.ts";
import {
  and,
  darkMode,
  hover,
...
const { Links$ } = component("Links", ReactRouter.Links);
const { Meta$ } = component("Meta", ReactRouter.Meta);
const { NavLink } = component("NavLink", ReactRouter.NavLink);
const { Outlet$ } = component("Outlet", ReactRouter.Outlet);
const { Scripts$ } = component("Scripts", ReactRouter.Scripts);
const { ScrollRestoration$ } = component(
  "ScrollRestoration",
...
const { ThemeSwitcher$ } = component("ThemeSwitcher", () => {
  ...
  return label(
    ...
    select(
      ...
      themes.map(theme =>
        option(
          { key: theme, style: { backgroundColor: "#fff", color: "#000" } },
          theme,
        ),
      ),
    ),
    svg(
      ...
      g(
        { style: pipe({}, on(darkMode, { display: "none" })) },
    ...
    ScreenReaderOnly$("Theme"),
  );
});

export function Layout({ children }: { children: React.ReactNode }) {
  return html(
    { lang: "en", "data-theme": "auto", suppressHydrationWarning: true },
    head$(
      script({
    ...
    body(
      ...
      footer(
        ...
        div$("©", ClientOnly$(new Date().getFullYear()), "Nick Saunders"),
        div({
          style: {
        ...
        div(
          ...
          Object.entries({
            ...
            TextLink({ key: href }, _a({ href }, children)),
          ),
        ),
      ),
      ScrollRestoration$(),
